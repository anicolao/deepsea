...
export function validEnvelope(value: unknown): value is Envelope {
  ...
  return (
    ...
    Number.isSafeInteger(value.reducerVersion) &&
    Number(value.reducerVersion) > 0 &&
    typeof value.rulesetVersion === "string" &&
    value.rulesetVersion.length > 0 &&
    value.rulesetVersion.length <= 32 &&
    typeof value.type === "string" &&
    value.type.length > 0 &&
    value.type.length <= 64 &&
    validId(value.actorUid) &&
    validId(value.clientId) &&
    Number.isSafeInteger(value.clientSeq) &&
    Number(value.clientSeq) >= 0 &&
    plain(value.payload) &&
    Object.keys(value.payload).length <= 16 &&
    jsonValue(value.payload) &&
    JSON.stringify(value.payload).length <= 2048
  );
}
export function supported(event: Envelope) {
  return (
    event.schemaVersion === VERSIONS.schemaVersion &&
    event.reducerVersion === VERSIONS.reducerVersion &&
    event.rulesetVersion === VERSIONS.rulesetVersion
  );
}
...
export const sameEnvelope = (a: Envelope, b: Envelope) =>
...
export function replay(gameId: string, input: unknown[]): Projection {
  ...
  for (const raw of input) {
    if (!plain(raw)) {
      result.diagnostics.push("Invalid event");
      continue;
    }
    const { id, createdAt, ...envelope } = raw;
    if (!validId(id) || !validStamp(createdAt) || !validEnvelope(envelope)) {
      result.diagnostics.push("Invalid envelope");
      continue;
    }
  ...
  for (const event of [...unique.values()].sort(compareEvents)) {
    ...
    if (!supported(event)) {
      result.blocked = true;
      result.diagnostics.push(`Unsupported version: ${event.id}`);
      continue;
    }
    if (result.blocked) continue;
    const room = apply(result.room, gameId, event);
    if (!room) {
      result.diagnostics.push(`Illegal action: ${event.id}`);
      continue;
    }
    result.room = room;
...
const keys = (payload: Record<string, unknown>, expected: string[]) =>
  Object.keys(payload).sort().join() === expected.sort().join();
const nameValid = (name: unknown): name is string =>
  typeof name === "string" &&
  name === name.trim() &&
  name.length > 0 &&
  name.length <= 40;
function apply(
  ...
  if (event.type === "game/created") {
    if (
      room ||
      event.id !== "created" ||
      !keys(p, ["gameId", "hostName"]) ||
      p.gameId !== gameId ||
      !nameValid(p.hostName)
    )
      return null;
    return {
      gameId,
      ...
      rosterRevision: event.id,
      members: [
        { uid: event.actorUid, name: p.hostName, seat: 1, ready: false },
      ],
    };
  }
  ...
  if (room.phase === "started" && room.dive && room.seed !== undefined) {
    ...
    if (
      event.type === "turn/rolled" &&
      !keys(p, ["direction", "expectedActionId"])
    )
      return null;
    if (
      event.type === "turn/landed" &&
      !keys(
        p,
        p.choice === "drop"
          ? ["choice", "unitId", "expectedActionId"]
          : ["choice", "expectedActionId"],
      )
    )
      return null;
    if (
      event.type === "dive/ordered" &&
      !keys(p, ["order", "expectedActionId"])
    )
      return null;
    if (event.type === "dive/continued" && !keys(p, ["expectedActionId"]))
      return null;
    const dive =
      event.type === "dive/continued"
        ? continueDive(room.dive, event.actorUid)
        : event.type === "dive/ordered"
          ? orderLost(room.dive, room.seed, event.actorUid, p.order)
          : turn(room.dive, room.seed, event.actorUid, event.type, p);
  ...
  if (event.type === "lobby/joined") {
    if (
      ...
      return null;
    const seat = [1, 2, 3, 4, 5, 6].find(
      (n) => !room.members.some((m) => m.seat === n),
    )!;
    return {
      ...changed,
      rosterRevision: event.id,
      members: [
        ...room.members.map((m) => ({ ...m, ready: false })),
        { uid: event.actorUid, name: p.name, seat, ready: false },
  ...
  if (event.type === "lobby/left") {
    ...
    return {
      ...changed,
      phase: event.actorUid === room.hostUid ? "closed" : "lobby",
      rosterRevision: event.id,
      members: room.members
        .filter((m) => m.uid !== event.actorUid)
        .map((m) => ({ ...m, ready: false })),
    };
  }
  if (event.type === "lobby/ready") {
    if (
      !member ||
      !keys(p, ["ready", "rosterRevision"]) ||
      typeof p.ready !== "boolean" ||
      p.rosterRevision !== room.rosterRevision ||
    ...
    return {
      ...
      members: room.members.map((m) =>
        m.uid === event.actorUid ? { ...m, ready: p.ready as boolean } : m,
      ),
    };
  }
  if (event.type === "game/started") {
    if (
      event.actorUid !== room.hostUid ||
      !keys(p, ["seed", "starterUid", "expectedActionId"]) ||
      p.expectedActionId !== room.lastActionId ||
      room.members.length < 2 ||
      !room.members.every((m) => m.ready) ||
      !room.members.some((m) => m.uid === p.starterUid) ||
      !Number.isInteger(p.seed) ||
      Number(p.seed) < 0 ||
      Number(p.seed) > 0xffffffff
    )
      return null;
    return {
      ...changed,
      phase: "started",
      starterUid: p.starterUid as string,
      seed: p.seed as number,
      dive: initialDive(
        p.seed as number,
        room.members.map((m) => m.uid),
        p.starterUid as string,
      ),
    };
  }
  return null;
