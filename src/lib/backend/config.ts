export type BackendConfig = { mode: 'local' | 'preview' | 'production'; projectId: string; apiKey: string; appId?: string; authDomain?: string; authHost?: string; firestoreHost?: string };
...
export function selectConfig(config: DeploymentConfig, url: URL): BackendConfig {
  ...
  if (['localhost', '127.0.0.1'].includes(url.hostname)) {
    ...
    return { ...config.local, mode: 'local', apiKey: 'local-emulator-key' };
  }
