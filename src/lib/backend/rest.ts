...
export async function appendEvent(
  ...
  if (
    ...
    !validId(pending.id) ||
    !validId(config.namespace ?? "local") ||
    !validEnvelope(pending.envelope)
  )
    throw new Error("Invalid event");
  const origin =
    config.mode === "local"
      ? "http://" + config.firestoreHost
      : "https://firestore.googleapis.com";
  const database =
    "projects/" + encodeURIComponent(config.projectId) + "/databases/(default)";
  const name =
    database +
  ...
  try {
    response = await fetch(origin + "/v1/" + database + "/documents:commit", {
      method: "POST",
      headers,
      body: JSON.stringify({
        writes: [
    ...
    /* The write may have reached the server. Read its stable ID before deciding. */
  }
  if (response?.ok) return;
  if (response && response.status !== 409 && response.status < 500)
    throw new Error("The server could not accept this move.");
  const check = await fetch(
    origin + "/v1/" + database + "/documents:batchGet",
    { method: "POST", headers, body: JSON.stringify({ documents: [name] }) },
  );
  if (!check.ok) throw new Error("The move is still unconfirmed.");
  const results = (await check.json()) as {
    found?: { name: string; fields: Record<string, Field> };
  }[];
