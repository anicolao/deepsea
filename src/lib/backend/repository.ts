...
export interface EventTransport {
  watch(
    gameId: string,
    next: (events: ConfirmedEvent[], synchronized: boolean) => void,
...
export type RoomState = Projection & {
...
export class RoomRepository {
  ...
  constructor(
    ...
    if (!validId(actorUid) || !validId(clientId) || !validId(namespace))
      throw new Error("Invalid repository identity");
    this.clientId = clientId;
    this.prefix = `deepsea:${namespace}:${actorUid}:`;
    this.sequence = Number(
      storage.getItem(`${this.prefix}sequence:${clientId}`) ?? 0,
    );
    if (!Number.isSafeInteger(this.sequence) || this.sequence < 0)
      throw new Error("Invalid saved sequence");
  }
  prepareCreation(gameId: string, hostName: string): PendingEvent {
    if (!validId(gameId) || !hostName.trim() || hostName.trim().length > 40)
      throw new Error("Enter a name between 1 and 40 characters.");
    const existing = this.pending(gameId);
    if (existing) {
      if (existing.envelope.payload.hostName !== hostName.trim())
        throw new Error(
          "Resolve the saved room creation before changing its name.",
        );
      return existing;
    }
    const clientSeq = ++this.sequence;
    if (!Number.isSafeInteger(clientSeq))
      throw new Error("Client sequence exhausted");
    this.storage.setItem(
      `${this.prefix}sequence:${this.clientId}`,
      String(clientSeq),
    );
    const pending: PendingEvent = {
      ...
      envelope: {
        ...VERSIONS,
        type: "game/created",
        payload: { gameId, hostName: hostName.trim() },
  ...
  prepareAction(
    ...
    if (this.pending(gameId))
      throw new Error("Confirm the pending action first.");
    const clientSeq = ++this.sequence;
  ...
  pending(gameId: string): PendingEvent | null {
    ...
    if (
      ...
      !validId(pending.id) ||
      !validEnvelope(pending.envelope) ||
      pending.envelope.actorUid !== this.actorUid ||
      !supported(pending.envelope)
    )
      throw new Error(
        "Saved room action is incompatible. Reload with a compatible app.",
      );
    return pending;
  }
  async submit(pending: PendingEvent) {
    if (
      !validId(pending.gameId) ||
      !validId(pending.id) ||
      (pending.envelope.type === "game/created" &&
        pending.envelope.payload.gameId !== pending.gameId) ||
      pending.envelope.actorUid !== this.actorUid ||
      !supported(pending.envelope) ||
      !validEnvelope(pending.envelope)
    )
    ...
    const saved = this.pending(pending.gameId);
    if (saved && !sameEnvelope(saved.envelope, pending.envelope))
      throw new Error("A different creation is already pending for this room.");
    this.storage.setItem(
      `${this.prefix}pending:${pending.gameId}`,
      JSON.stringify(pending),
    );
  ...
  watch(gameId: string, next: (state: RoomState) => void) {
    ...
    const notify = () =>
      next({
        ...last,
        synchronized: lastSynchronized,
        pending: true,
        error: null,
      });
    const listeners =
      this.pendingListeners.get(gameId) ?? new Set<() => void>();
    listeners.add(notify);
    this.pendingListeners.set(gameId, listeners);
    const stop = this.transport.watch(
      gameId,
      (events, synchronized) => {
        ...
        try {
          pending = this.pending(gameId) ?? this.submitted.get(gameId) ?? null;
        } catch {
          next({
            ...projection,
            blocked: true,
        ...
        if (pending && confirmed && validStamp(confirmed.createdAt)) {
          ...
          if (sameEnvelope(envelope, pending.envelope)) {
            if (
              !projection.acceptedIds.includes(confirmed.id) &&
              !projection.blocked
            )
              error =
            ...
            error =
              "Room ID collision: the saved action differs from the confirmed room.";
        }
      ...
      (error) =>
        next({
          ...
          synchronized: false,
          pending:
            this.storage.getItem(`${this.prefix}pending:${gameId}`) !== null,
          error: error.message,
        }),
    );
    return () => {
      ...
      if (!listeners.size) this.pendingListeners.delete(gameId);
    };
  }
}
export const eventId = (clientId: string, sequence: number) => {
  if (!validId(clientId) || !Number.isSafeInteger(sequence) || sequence < 1)
