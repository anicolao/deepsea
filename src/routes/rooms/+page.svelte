...
<main class:playing={room?.phase === "started" && !!me}>
  ...
  {#if room?.phase !== "started" || !me}<h1>Gather your crew</h1>{/if}
  ...
  {:else if !roomId && joining}
    <form on:submit|preventDefault={openInvite}>
      ...
      <p><a href={base + "/rooms/"}>Create a room instead</a></p>
    </form>
  {:else if !roomId}
    <form on:submit|preventDefault={create}>
      <h2>Create a room</h2>
      <p>Bring 2–6 friends, each on their own browser.</p>
  ...
  {:else if room?.phase === "closed"}
    <section>
      ...
      <a href={`${base}/rooms/`}>Create another room</a>
    </section>
    ...
    <section aria-label="Room lobby">
      ...
      {#if me}
        <div class="actions">
          ...
          ><button class="secondary" on:click={copy}>Copy invite</button>
        </div>
        ...
        {#if closing}<div role="group" aria-label="Confirm room closure">
            <p>Leaving closes this room for everyone.</p>
            <div class="actions">
              <button on:click={() => act("lobby/left", {})} disabled={!enabled}
                >Close room</button
              ><button class="secondary" on:click={() => (closing = false)}
                >Stay</button
              >
            </div>
          </div>
        {:else}<button
            class="quiet"
            disabled={!enabled}
            on:click={() => (host ? (closing = true) : act("lobby/left", {}))}
            >Leave room</button
          >{/if}
  ...
  {#if state?.pending && !state.blocked}<button
      on:click={retry}
