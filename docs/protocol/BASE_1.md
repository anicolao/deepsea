...
The initial envelope uses integer `schemaVersion: 1`, integer `reducerVersion: 1`, and `rulesetVersion: "base-1"`. Missing/invalid envelopes are diagnosed and ignored. A structurally valid incompatible version blocks further interaction; it is never interpreted as version 1. The current reducer accepts room creation only. Future gameplay event types must be implemented and tested before enabling them in the Firestore rules.
...
Room IDs are cryptographically random UUIDs and the creation document ID is always `created`. Future action IDs use the per-tab client ID and monotonic sequence. A page load allocates a fresh cryptographic client ID; previously saved pending actions retain their exact original ID, sequence, actor, versions, and payload across reloads. Counters and pending records are namespaced by Firebase project and authenticated UID. This avoids cloned-tab sequence collisions without rewriting an earlier pending action.
