...
const pending: PendingEvent = {
  ...
  envelope: {
    ...
    clientId: "tab",
    clientSeq: 1,
    payload: { ready: true, rosterRevision: "created" },
  },
};
const name =
  "projects/demo-deepsea/databases/(default)/documents/environments/local/games/room/events/tab_1";
...
it("creates a new event atomically with a server timestamp in one round trip", async () => {
  ...
  expect(decode({ mapValue: { fields: write.update.fields } })).toEqual(
    pending.envelope,
  );
});
it("resolves lost replies and existing IDs by reading and comparing the exact event without rewriting it", async () => {
  for (const lost of [true, false]) {
    const fetcher = vi.fn();
    if (lost) fetcher.mockRejectedValueOnce(new TypeError("reply lost"));
    else fetcher.mockResolvedValueOnce(new Response("{}", { status: 409 }));
      ...
      Response.json([
        {
          found: {
            name,
            fields: {
              ...encode(pending.envelope).mapValue!.fields,
              createdAt: { timestampValue: "2026-09-11T00:00:00Z" },
            },
          },
        },
      ]),
    ...
    expect(fetcher.mock.calls[1][0]).toContain("/documents:batchGet");
  ...
  for (const mode of ["different", "missing", "denied"]) {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        new Response("{}", { status: mode === "denied" ? 403 : 409 }),
      );
    fetcher.mockResolvedValueOnce(
      Response.json(
        mode === "missing"
          ...
          : [
              {
                found: {
                  ...
                  fields: encode({
                    ...pending.envelope,
                    payload: { ready: false, rosterRevision: "created" },
                  }).mapValue!.fields,
                },
              },
            ],
    ...
    vi.stubGlobal("fetch", fetcher);
    await expect(appendEvent(config, "test-token", pending)).rejects.toThrow();
    expect(fetcher).toHaveBeenCalledTimes(mode === "denied" ? 1 : 2);
  }
  const fetcher = vi.fn();
  vi.stubGlobal("fetch", fetcher);
  await expect(
    appendEvent(config, "test-token", { ...pending, id: "wrong/path" }),
