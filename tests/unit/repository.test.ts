...
import {
  ...
  type RoomState,
  type Store,
} from "../../src/lib/backend/repository";
import {
  type PendingEvent,
  type ConfirmedEvent,
...
function fixture() {
  ...
  const transport: EventTransport = {
    ...
    async create(pending) {
      ...
      if (existing && existing.actorUid !== pending.envelope.actorUid)
        throw new Error("collision");
      if (!existing) {
        creates++;
        events.set(pending.id, {
          ...pending.envelope,
          id: pending.id,
          createdAt: { seconds: 1, nanoseconds: 500 },
        });
      }
      if (loseAck) {
        loseAck = false;
  ...
  return {
    ...
    publish: (synced = true) => notify([...events.values()], synced),
    creates: () => creates,
  };
}
it("persists the exact action before sending, surviving a reload and lost acknowledgement", async () => {
  ...
  f.loseAck();
  await expect(f.repository.submit(pending)).rejects.toThrow(
    "Acknowledgement lost",
  );
  const afterReload = new RoomRepository(
    f.transport,
    f.storage,
    "mira",
    "tab2",
...
it("isolates identities and projects; allocates distinct per-tab event IDs", () => {
  ...
  expect(
    f.repository.prepareCreation("other-room", "Mira").envelope.clientSeq,
  ).toBe(2);
});
it("rejects changed pending payloads and mismatched submission actors", async () => {
  ...
  const forged = {
    ...pending,
    envelope: { ...pending.envelope, actorUid: "sol" },
  } as PendingEvent;
  ...
  await expect(
    f.repository.submit({
      ...
      envelope: {
        ...pending.envelope,
        payload: { gameId: "room", hostName: "Other" },
      },
    }),
...
it("blocks a corrupt or incompatible saved action without throwing from a snapshot callback", () => {
  ...
  for (const raw of [
    "{bad json",
    JSON.stringify({ id: "created", gameId: "room", envelope: {} }),
  ]) {
...
it("reports a confirmed but rejected lobby action and preserves its identity across retry", async () => {
  ...
  f.publish();
  const pending = f.repository.prepareAction("room", "game/started", {
    seed: 1,
    starterUid: "mira",
    expectedActionId: "created",
  });
...
it("keeps moves pending from preparation until the confirmed projection arrives", async () => {
  ...
  expect(state?.pending).toBe(true);
  await f.repository.submit(action);
  expect(state?.pending).toBe(true);
  f.publish();
  expect(state?.pending).toBe(false);
  expect(state?.room?.members[0].ready).toBe(true);
  stop();
  f.repository.prepareAction("room", "lobby/ready", {
    ready: false,
    rosterRevision: "created",
  });
  expect(state?.pending).toBe(false);
