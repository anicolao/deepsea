...
test("friends create, join, ready and start a room through real browser actions", async ({
  players,
}, info) => {
  ...
  const thirdSteps = new TestSteps(
    third,
    info,
    "Join the crew",
  ...
  await hostSteps.step("created", "The host invites friends", [
    {
      ...
      assert: async () => {
        ...
        await expect(
          host.getByText("Invite at least one friend to start."),
        ).toBeVisible();
      },
    },
  ]);
  ...
  await guestSteps.step("joined", "A friend joins and readies up", [
    {
      description:
      ...
      assert: async () => {
        ...
        await expect(
          guest.getByRole("button", { name: "Not ready", exact: true }),
        ).toBeEnabled();
        await expect(
          guest.getByRole("button", { name: "Start dive" }),
        ).toHaveCount(0);
      },
    },
  ]);
  ...
  await thirdSteps.step("joined", "A third friend joins", [
    {
      description:
        "Ada is seated in the same three-person crew and all readiness has reset.",
      assert: async () => {
        await expect(
          third.getByRole("list", { name: "Crew" }).getByRole("listitem"),
        ).toHaveCount(3);
        await expect(
          third.getByText("Ada (You)", { exact: true }),
        ).toBeVisible();
        await expect(
    ...
    [
      {
        ...
        assert: async () => {
          ...
          await expect(
            third.getByRole("heading", { name: "Sol’s turn", exact: true }),
          ).toBeVisible();
        },
      },
    ],
