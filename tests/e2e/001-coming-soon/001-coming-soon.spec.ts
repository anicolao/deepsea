...
test("visitors see the home screen and can read the game brief", async ({
  page,
}, info) => {
  ...
  await steps.step(
    ...
    [
      {
        description: "The page title and main heading identify Deep Sea.",
        assert: async () => {
      ...
      {
        description: "Gather your crew and the three game facts are visible.",
        assert: async () => {
          await expect(
            page.getByRole("link", { name: "Create room", exact: true }),
          ).toBeVisible();
          await expect(
      ...
      },
      {
        ...
        assert: async () => {
          await expect(
            page.getByRole("button", { name: "About the game" }),
          ).toBeEnabled();
          await expect(
            page.getByRole("link", { name: "Create room", exact: true }),
          ).toBeVisible();
  ...
  await steps.step("game-brief", "A keyboard user can learn the game", [
    {
      ...
      assert: async () => {
        const dialog = page.getByRole("dialog", {
          name: "Treasure is only yours if you make it back.",
        });
    ...
    {
      description: "The brief explains treasure risk in player language.",
      assert: async () => {
        await expect(page.getByRole("dialog")).toContainText(
          "Carry only what you can bring home.",
        );
      },
    },
    {
      description: "The close control receives focus and is keyboard operable.",
      assert: async () => {
        await expect(
  ...
  await steps.step("return", "Closing the brief restores the visitor’s place", [
    ...
    {
      description: "The create-room link remains visible.",
      assert: async () => {
        await expect(
          page.getByRole("link", { name: "Create room", exact: true }),
        ).toBeVisible();
