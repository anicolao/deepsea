...
test("two players can leave, rejoin and start while late invites are rejected", async ({
  players,
}, info) => {
  ...
  const lateSteps = new TestSteps(
    late,
    info,
    "An invite after start",
  ...
  await guestSteps.step("left", "Leaving releases the seat", [
    {
      description: "The guest can rejoin and the host is alone again.",
      assert: async () => {
        await expect(
  ...
  await hostSteps.step("started", "Two ready friends start", [
    {
      description:
        "The saved start contains both seats and Mira as first diver.",
      assert: async () => {
        await expect(
          host.getByRole("heading", { name: "Your turn", exact: true }),
        ).toBeVisible();
        await expect(
          host.getByRole("button", { name: "Roll dice" }),
        ).toBeEnabled();
        await expect(
  ...
  await lateSteps.step("late-invite", "Late arrivals get a clear explanation", [
    {
      description:
        "The started room rejects a new player and offers a new room.",
      assert: async () => {
        await expect(
          late.getByRole("heading", { name: "This dive has already started" }),
        ).toBeVisible();
        await expect(
          late.getByRole("button", { name: "Join room" }),
        ).toHaveCount(0);
        await expect(
          late.getByRole("link", { name: "Create another room" }),
        ).toBeVisible();
      },
    },
  ]);
