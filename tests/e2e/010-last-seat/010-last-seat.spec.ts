...
test("two friends compete for the final seat and only one joins", async ({
  players,
}, info) => {
  const crew: {
  ...
  for (const name of ["Mira", "Sol", "Kai", "Luz", "Pip"]) {
    const page = await players.create();
    crew.push({
      page,
      name,
      steps: new TestSteps(
  ...
  const a = await players.create(),
    b = await players.create();
  for (const contender of [a, b]) {
    await contender.goto(host.url());
    await contender.getByLabel("Your name").fill("Nori");
    await contender.getByRole("button", { name: "Join room" }).focus();
  }
  ...
  await expect(
    a.getByRole("list", { name: "Crew" }).getByRole("listitem"),
  ).toHaveCount(6);
  await expect(
    b.getByRole("list", { name: "Crew" }).getByRole("listitem"),
  ).toHaveCount(6);
  const winner = (await a.getByText("Nori (You)", { exact: true }).isVisible())
    ? a
    : b;
  const loser = winner === a ? b : a;
  const ws = new TestSteps(
    winner,
    info,
    "The final seat",
  ...
  await ws.step("seated", "One final diver joins", [
    {
      description: "The winner has exactly one seat and readiness controls.",
      assert: async () => {
        await expect(
          winner.getByText("Nori (You)", { exact: true }),
        ).toHaveCount(1);
        await expect(
          winner.getByRole("button", { name: "Ready up" }),
        ).toBeEnabled();
      },
    },
  ]);
  await ls.step("full", "The other invitation cannot claim a seat", [
    {
      description:
        "The losing friend is not seated and can create another room.",
      assert: async () => {
        await expect(
          loser.getByRole("heading", { name: "Room full", exact: true }),
        ).toBeVisible();
        await expect(
          loser.getByRole("button", { name: "Ready up" }),
        ).toHaveCount(0);
        await expect(
          loser.getByRole("link", { name: "Create another room" }),
        ).toBeVisible();
      },
    },
  ]);
  for (const friend of crew) {
    await friend.steps.step("full-crew", "The crew has exactly six seats", [
      {
        description: "All seated friends see the same six names.",
        assert: async () => {
          await expect(
            friend.page
              .getByRole("list", { name: "Crew" })
              .getByRole("listitem"),
          ).toHaveCount(6);
          await expect(
            friend.page
              .getByRole("list", { name: "Crew" })
              .getByText("Nori", { exact: true }),
          ).toHaveCount(1);
        },
      },
    ]);
    if (friend.page !== host) friend.steps.finish();
  }
  ws.finish();
  ls.finish();
  await Promise.all(
    [...crew.map((friend) => friend.page), winner].map((page) =>
  ...
  await host
    ...
    .click();
  const hostSteps = crew.at(0)!.steps;
    ...
    [
      {
        ...
        assert: async () => {
          await expect(
            host
              .getByRole("dialog", { name: "Your crew & cargo" })
              .getByRole("listitem"),
          ).toHaveCount(6);
          await expect(host.getByRole("dialog")).toContainText(
            "Unit 1: 1 tile, level 1",
          );
          await expect(host.getByRole("dialog")).toContainText("Nori");
          await expect(host.getByRole("dialog")).not.toContainText(
            /seed|actorUid|createdAt/,
          );
        },
      },
    ],
