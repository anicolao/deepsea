...
The coming-soon screen and a minimal room setup are implemented. Local anonymous identities can create an immutable saved room, reload as its host, and read it from another browser. Joining, readiness, and gameplay are not implemented yet. The production build is covered by keyboard and screenshot tests on phone and desktop.
...
`static/backend.json` selects the demo backend only on loopback hosts and contains separate, currently unset preview and production configurations. Hosted room setup therefore reports that it is unavailable; it never falls back to emulators or a production project. Provisioning hosted backends remains step 10 of the plan. See [Firebase's emulator setup](https://firebase.google.com/docs/emulator-suite/install_and_configure) and [Auth emulator connection](https://firebase.google.com/docs/emulator-suite/connect_auth).
