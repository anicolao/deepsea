...
`static/backend.json` selects the demo backend only on loopback hosts and contains separate, currently unset preview and production configurations. Hosted room setup therefore reports that it is unavailable; it never falls back to emulators or a production project. A configured live backend and passing deployed browser journeys are required for this PR and every future PR; they cannot be deferred to step 10. See [Firebase's emulator setup](https://firebase.google.com/docs/emulator-suite/install_and_configure) and [Auth emulator connection](https://firebase.google.com/docs/emulator-suite/connect_auth).
