...
The shared fixture owns additional browsers through `players.create()`. Each inherits canonical rendering, two-second action/navigation deadlines, network restrictions, and error checks. Each page needs its own named TestSteps view. Native anonymous UIDs vary between isolated runs; scenarios assert identity relationships and use fixed display names. Visible room codes use reproducible initialization in a fresh isolated namespace for each scenario execution. Rules randomness has fixed golden seed fixtures. Each browser scenario creates its own room and identities in the freshly started demo project; emulator-rules tests use a separate demo project.
...
The shared fixture supplies reproducible initialization randomness through the app's `backend.json` response: seed 2026 by default, or an explicitly validated unsigned seed passed to `players.create(seed)`. All identities, rooms, events and board transitions still come from normal player actions and the real SDK. The fixture also supplies one random UUID `testRun` per scenario execution, shared by its players. Local and preview configuration validate it and append it beneath the deployment namespace; it cannot select another deployment. In these isolated runs only, the initialization seed also drives the normal room-code generator, so visible codes can be compared exactly without collisions with previous runs. Production rejects both overrides. No room or action data is inserted. The source guard permits only this exact configuration fulfillment; scenarios cannot fulfill requests or inject state. `TestSteps.gameLayout()` switches a lobby walkthrough to the stricter scrollable-path and persistent-action checks after start.
...
The multiplayer-turn journey reads a newly created room’s five-letter code directly from the lobby beside Copy invite and joins from another browser with lowercase entry. It checks the waiting player’s dice, air charge and current decision before its full screenshot. The room-foundation journey also checks that the visible code matches the copied invitation and is visible to the joined guest. Codes are allocated by the app’s normal creation flow, using the isolated initialization configuration described above; anonymous identities remain real. Layout failures name overlapping controls to make responsive regressions actionable.
