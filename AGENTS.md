...
The MVP will be a multiplayer game on the web. Its design and detailed scope are not yet defined. Do not describe it as a shared-device game or invent MVP requirements.

Record every user prompt concerning this project verbatim in `PROMPTS.md`, including corrections and follow-up requests. Append each prompt once, in conversation order, under the next `## Prompt N` heading. Preserve spelling, punctuation, capitalization, and line breaks. Do not include assistant messages or automatically supplied environment context.
