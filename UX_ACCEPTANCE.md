...
Five-letter room codes are always visible in large type beside Copy invite in the lobby and accepted case-insensitively on arrival; existing same-table links remain valid. Code allocation uses an atomic immutable room creation, retries confirmed collisions, and retains uncertain writes for recovery. The waiting view shows the latest roll and destination, actual air charged before any pickup/drop, current decision and recent confirmed action. Follow turn tracks moves, with smooth scrolling, dice/arrival animations and a transitioning oxygen meter when motion is allowed. Reduced-motion mode keeps all feedback immediately readable. Crew load is a forecast at current carried units, excluding returned divers; it is not an automatic time-based charge.
